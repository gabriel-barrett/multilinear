...
impl<F: HashableField + NttField> QueryProof<F> {
    pub fn verify(
        ...
        // remember to verify the `index`!
        // the random index is generated by the transcript
        // remember to update the transcript EXACTLY like the prover did
        let mut current_gen = gen;
        for i in 0..self.paths.len() {
            // this is the last case
            if i == self.paths.len() - 1 {
                // similar to else case, except that `next_value` is `last_element`
            } else {
